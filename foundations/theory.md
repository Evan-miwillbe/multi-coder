...
**共识**：两者都对，取决于任务类型。
Multi-coder 采用 Read/Write 分离：subagent 默认不改产品源码，main CC 是唯一源码 writer；只有在 worktree 隔离、文件所有权明确、用户批准时，才允许 implementer-worktree 并行写。状态目录产出可由 subagent 写入或由主 CC 代写。
