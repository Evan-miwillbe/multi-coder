...
**防御**：主 CC 是唯一 writer，subagent 只读。
...
**防御**：subagent 结果落到 filesystem，仅回传 reference（handoff 150-200字）。
...
**影响**：整轮工作丢失。
**防御**：每个 subagent 独立完成一个 write 操作后才算安全，失败后重新 spawn 而非全部重来。

### 8. Sub-sub-agent 不支持（GitHub #19077）
**表现**：subagent 内部尝试 spawn 自己的 subagent。
**影响**：crash。
**防御**：subagent 只有 read-only 权限，不 spawn。
...
### 10. AI 生成代码的隐性 bug（r/programming, Jan 2026）
**表现**：AI 生成的代码"看起来能跑"但有隐蔽安全漏洞/逻辑错误。
**影响**：Amazon 2026 AI-code outages，数百万损失。
**防御**：Security Gate + Adversarial review 每次 write 后自动触发。
